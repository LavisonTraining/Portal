...
function getDeviceName(){
  ...
  if (ua.indexOf("Edg/") !== -1){ const m = ua.split("Edg/")[1]; browser = "Edge " + (m ? m.split(".")[0] : ""); }
  else if (ua.indexOf("Chrome/") !== -1){ const m = ua.split("Chrome/")[1]; browser = "Chrome/" + (m ? m.split(".")[0] : ""); }
  else if (ua.indexOf("Firefox/") !== -1){ const m = ua.split("Firefox/")[1]; browser = "Firefox " + (m ? m.split(".")[0] : ""); }
  else if (ua.indexOf("Safari/") !== -1 && ua.indexOf("Version/") !== -1){ const m = ua.split("Version/")[1]; browser = "Safari " + (m ? m.split(".")[0] : ""); }
...
window.addAttendance = function(){
  ...
  block.innerHTML = `
    <button type="button" class="btn remove-btn" onclick="this.parentElement.remove(); (${checkFormValidity.toString()})();">❌</button>
    <label>📅 التاريخ:
  ...
  attendanceContainer.appendChild(block);
  checkFormValidity();
};
...
window.addAbsence = function(){
  ...
  absenceContainer.appendChild(block);
  checkFormValidity();
};
